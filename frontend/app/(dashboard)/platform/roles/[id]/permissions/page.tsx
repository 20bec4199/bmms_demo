...
export default function PlatformManageRolePermissionsPage() {
  const router = useRouter();
  const searchParams = useSearchParams();
  const orgId = searchParams.get('orgId');
  const { id: roleId } = useParams();
  
  const { data: rolesData, isLoading: isLoadingRoles } = useGetRolesQuery(orgId ? { orgId } : {}, { skip: !orgId });
  ...
  if (!orgId) return <div className="p-8 text-center text-red-500">No organization context provided. Please go back to roles.</div>;
  if (isLoadingRoles || isLoadingPerms) return <div className="p-8 text-center text-gray-500">Loading configurations...</div>;
  if (!role) return <div className="p-8 text-center text-red-500">Role not found in the selected organization.</div>;
  ...
  const handleToggle = async (permissionId: string, isAssigned: boolean) => {
    ...
    try {
      if (isAssigned) {
        await removePerm({ roleId, permissionId, orgId }).unwrap();
      } else {
        await assignPerm({ roleId, permissionId, orgId }).unwrap();
      }
    } catch (err: any) {
  ...
  return (
    <div>
      <div className="mb-4">
        <Link href="/platform/roles" className="inline-flex items-center text-sm font-medium text-gray-500 hover:text-gray-700 dark:text-gray-400 dark:hover:text-gray-200">
      ...
      <PageHeader 
        ...
        description="Toggle the exact actions this role is authorized to perform for the organization."
      />
              ...
              try {
                const promises = subjectPerms.map((p: any) => {
                  ...
                  if (allAssigned && isAssigned) {
                    return removePerm({ roleId, permissionId: p.id, orgId }).unwrap();
                  } else if (!allAssigned && !isAssigned) {
                    return assignPerm({ roleId, permissionId: p.id, orgId }).unwrap();
                  }
